...
function addTax(array) {
    ...
    let tax = Math.round(array[1] * 0.0925 * pow) / pow;

    array.splice(1, 0, tax);
    return array;
}
...
function getEvens(startNum, endNum) {
    // Optimization: instead of checking if every number is even in loop.
    // just have i be an even number initially and increment i by two.
    let i = startNum % 2 == 0 ? startNum : startNum + 1;
